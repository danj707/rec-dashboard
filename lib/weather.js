...
/* ── Current conditions for an org's own dashboard ─────────────────────────
   THIS FILE EXISTS TWICE, and deliberately: rental-report/lib/weather.js is
   its twin, byte for byte. Two deployed services in two repositories cannot
   share a module, and no CI job here can assert the other copy agrees — so the
   rule is the one that governs every other duplicated definition in these two
   projects: CHANGE BOTH, or neither. A sky code that means rain on the report
   and overcast on the dashboard is the two-surfaces-disagreeing bug, one repo
   over.
